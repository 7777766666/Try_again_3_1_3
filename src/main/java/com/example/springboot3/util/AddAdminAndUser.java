...
@Component
public class AddAdminAndUser {
    private final RoleService roleService;
    private final UserService userService;
    final BCryptPasswordEncoder bCryptPasswordEncoder;
    @Autowired
    public AddAdminAndUser(RoleService roleService, UserService userService, BCryptPasswordEncoder bCryptPasswordEncoder) {
    ...
    private void postConstruct() {
        ...
        User admin = new User("admin", "admin", "admin@admin.ru", "admin", roles_admin);
        userService.addUser(admin);
        Set<Role> roles_user = new HashSet<>();
        roles_user.add(roleService.getRoleByName("ROLE_USER"));
        User user = new User("user", "user",
                "user@user.ru", "user",  roles_user);
